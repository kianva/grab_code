...
function detect($var){
	$encode = mb_detect_encoding($var,array('GB2312','GBK','UTF-8'));
	...
	else if( $encode == 'CP936' ) $real_encode = 'UTF-8';
	else { $real_encode = ''; }
	return $real_encode;
}
...
{
	ob_start();
	$error_logs = "/home/wwwroot/google_search/error_logs.txt";
	$logs = "/home/wwwroot/google_search/logs.txt";
	$query_backup = $query ;
	$queue_url = array();
	$queue_url2 = array();
	//$dir = dirname(__FILE__)."/html/";
	$dir = "/home/wwwroot/google_search/html/";
	if( !file_exists($dir) )
	{
		mkdir($dir,0777,true);
	}
					...
					if(preg_match_all($pattern_p,$string,$match_p)>0)
					{
						...
						if( $mark%2 == 0 )
						{
							...
							if( $encode_article2 == 'GB2312' || $encode_article2 == 'gb2312' )
							{
								...
								$query = @iconv($real_encode_query,'GB2312//IGNORE',$query);
								//$query = mb_convert_encoding($query,'GB2312',$real_encode_query);
								if( $encode_article1 != $encode_article2 )
								{
									$string_p_1 = @iconv($encode_article1,'GB2312//IGNORE',$string_p_1);
							...
							else if( $encode_article2 == 'GBK' || $encode_article2 == 'gbk' )
							{
								...
								$real_encode_query = detect($query);
								$query = @iconv($real_encode_query,'GBK//IGNORE',$query);
								//$query = mb_convert_encoding($query,'GBK',$real_encode_query);
								if( $encode_article1  != $encode_article2 )
								{
									$sting_p_1 = @iconv($encode_article1,'GBK//IGNORE',$string_p_1);
							...
							else if( $encode_article2 == 'UTF-8' || $encode_article2 == 'utf-8' )
							{
								...
								if( $encode_article1 != $encode_article2)
								{
									$string_p_1 = @iconv($encode_article1,"UTF-8//IGNORE",$string_p_1);
	...
	<?php			
							...
							//$query = @iconv('UTF-8','GB2312',$query_backup); //windows保存是GB2312 ,but linus is utf8
							$query = $query_backup ;
							...
							{
								$time = date("Y-m-d H:i");
								$string_p = addslashes($string_p);
								if( $meta == 'UTF-8' || $meta == 'utf-8' )
								{
									mysql_query('set names utf8');
									$encode_string_p_x = detect( $string_p );
									if( $encode_string_p_x != 'UTF-8')
									{
										$string_p = iconv( $encode_string_p_x ,'UTF-8',$string_p );
									}
									$sql = "insert into article(title,content,time,source,encode) values('".$query_backup."','".$string_p."','".$time."','".$url."','".$meta."')";
								}
								if( $meta == 'GBK' || $meta == 'GB2312' || $meta == 'gb2312' || $meta == 'gbk' )
								{
									mysql_query("set names gbk");
									$encode_query_x = detect($query) ;
									$query = iconv($encode_query_x,'GBK',$query);
									$encode_string_p_x = detect($string_p);
									if( $encode_string_p_x != 'GBK' )
									{
										$string_p = iconv($encode_string_p_x,'GBK',$string_p);
									}
									$sql = "insert into article(title,content,time,source,encode) values('".$query."','".$string_p."','".$time."','".$url."','".$meta."')";
								}
								mysql_query($sql);
								if( mysql_affected_rows() > 0 )
								{
									$success = '插入成功-'.$time."--".$query_backup."_".$page."_".$i."\r\n";
									$fp2 = fopen($logs,'a+');
									fwrite($fp2,$success);
									fclose($fp2);
								}
								else{
									$error = "插入失败:".$time."--".$query_backup."_".$page."_".$i."\r\n";
									$fp3 = fopen($error_logs,'a+');
									fwrite($fp3,$error);
									fclose($fp3);
								}
							}
...
//$path = "d:/gamelist/key/";
$path = "/home/wwwroot/google_search/source/";
...
if( is_dir($path) )
{
	if( $dh = opendir($path) )
	{
		...
		{
			//$file = iconv('GB2312','UTF-8',$file);   //windows
